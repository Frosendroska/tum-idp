...
from configuration import (
    DEFAULT_OBJECT_KEY, RAMP_STEP_MINUTES, RAMP_STEP_CONCURRENCY, SYSTEM_BANDWIDTH_MBPS,
    PLATEAU_THRESHOLD, MAX_ERROR_RATE, MIN_REQUESTS_FOR_ERROR_CHECK, MAX_CONSECUTIVE_ERRORS, PROGRESS_INTERVAL, MAX_RETRIES, MAX_CONCURRENCY
)
...
class Ramp:
    ...
    def __init__(self, worker_pool, initial_concurrency: int = 8, 
        ...
        logger.info(f"Initialized ramp with plateau detection: {initial_concurrency} -> ?, step {self.ramp_step} every {self.step_duration_seconds}s, system limit: {self.system_bandwidth_mbps} Mbps")
    ...
    def execute_step(self, concurrency: int, step_id: str):
        ...
        # Adjust worker pool to target concurrency (reuses existing workers)
        self.worker_pool.start_workers(concurrency, self.object_key)
        
        # Begin new ramp step
        self.worker_pool.begin_phase(step_id, concurrency)
        
        # Give workers a moment to start making requests
        time.sleep(2)
        
        # Set step start time
        self.worker_pool.set_phase_step_time(step_id, time.time())
