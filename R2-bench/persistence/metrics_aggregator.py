...
class MetricsAggregator:
    ...
    def get_step_stats(self, phase_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            ...
            if not step_records:
                logger.warning(
                    f"No records found for phase {phase_id} after step start time"
                )
                return None
    ...
    def _compute_step_statistics(
        self, phase_id: str, records: List[BenchmarkRecord], step_start_ts: float
    ) -> Dict[str, Any]:
        ...
        if successful_requests > 0:
            ...
            throughput_mbps = (
                (total_bytes * 8) / (step_duration * 1_000_000)
                if step_duration > 0
                else 0
            )
        else:
        ...
        logger.debug(
            f"Computed step stats for {phase_id}: {throughput_mbps:.1f} Mbps, "
