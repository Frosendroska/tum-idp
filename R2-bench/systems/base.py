...
class ObjectStorageSystem:
    ...
    def upload_object_streaming(
        ...
        try:
                            ...
                            if etag:
                                ...
                                with error_lock:
                                    upload_errors.append(
                                        f"Failed to upload part {part_num}"
                                    )
            ...
            logger.info(
                f"Successfully uploaded {key} with streaming concurrent multipart upload"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to upload {key} with streaming concurrent multipart upload: {e}"
            )
