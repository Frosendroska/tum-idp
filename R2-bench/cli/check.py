...
class CapacityChecker:
    ...
    def _start_workers_for_phase(self, concurrency: int):
        ...
        for i in range(workers_needed):
            worker = threading.Thread(
                target=self._worker_loop, args=(i,), name=f"Worker-{i}"
            )
            worker.daemon = True
            worker.start()
        ...
        logger.info(
            f"Started {len(self.workers)} workers, semaphore permits: {self.semaphore.available_permits()}"
        )
    ...
    def _worker_loop(self, worker_id: int):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                logger.info(
                    f"Worker {worker_id} attempting to acquire semaphore permit (available: {self.semaphore.available_permits()})"
                )
                if not self.semaphore.acquire(timeout=1.0):
                    logger.warning(
                        f"Worker {worker_id} failed to acquire semaphore permit (available: {self.semaphore.available_permits()})"
                    )
                    continue

                logger.info(
                    f"Worker {worker_id} acquired semaphore permit (remaining: {self.semaphore.available_permits()})"
                )
                ...
                logger.info(
                    f"Worker {worker_id} assigned to phase: '{phase_id}' (target_concurrency: {phase_info.get('target_concurrency', 'N/A')})"
                )
                ...
                while retry_count < max_retries:
                    try:
                        logger.debug(
                            f"Worker {worker_id} attempting download: {range_start}-{range_start + range_length - 1} bytes"
                        )
                        ...
                        if data and len(data) > 0:
                            http_status = 200
                            logger.debug(
                                f"Worker {worker_id} successful download: {len(data)} bytes in {latency_ms:.1f}ms"
                            )
                            break
                        else:
                            retry_count += 1
                            logger.warning(
                                f"Worker {worker_id} empty data on retry {retry_count}/{max_retries}"
                            )
                            if retry_count < max_retries:
                                time.sleep(
                                    1.0
                                )  # Longer delay before retry for large downloads
                    except Exception as e:
                        retry_count += 1
                        logger.warning(
                ...
                if http_status != 200:
                    ...
                    if consecutive_errors >= MAX_CONSECUTIVE_ERRORS:
                        logger.error(
                            f"Worker {worker_id} stopping due to {consecutive_errors} consecutive errors"
                        )
                        break
                else:
                    consecutive_errors = 0
                ...
                if self.metrics_aggregator.get_total_records() % PROGRESS_INTERVAL == 0:
                    logger.info(
                        f"Progress: {self.metrics_aggregator.get_total_records()} requests completed "
    ...
    def _run_phase(self, duration_seconds: float) -> Dict[str, Any]:
        ...
        logger.info(
            f"Running phase {phase_id} for {duration_seconds:.1f}s with target concurrency {target_concurrency}"
        )
        ...
        if step_stats:
            logger.info(
                f"Phase {phase_id} completed: {step_stats['throughput_mbps']:.1f} Mbps, "
            ...
            with self.metrics_aggregator.lock:
                phase_records = [
                    r for r in self.metrics_aggregator.records if r.phase_id == phase_id
                ]
                logger.warning(
                    f"Debug: Found {len(phase_records)} records for phase {phase_id}"
                )
                if phase_records:
                    logger.warning(
                        f"Debug: First record start_ts: {phase_records[0].start_ts}, step_start_ts: {self.metrics_aggregator.phase_step_times.get(phase_id, 'Not set')}"
                    )
    ...
    def check_capacity(self, object_key: str = None):
        ...
        try:
            ...
            while (
                current_concurrency <= MAX_CONCURRENCY and not self.stop_event.is_set()
            ):
                step_count += 1
                ...
                logger.info(
                    f"Starting ramp step {step_count}: {current_concurrency} connections"
                )
            ...
            logger.info(
                f"Plateau detected: {plateau_reached if 'plateau_reached' in locals() else False}"
            )
            logger.info(
                f"Plateau reason: {reason if 'reason' in locals() else 'Max concurrency reached'}"
            )
